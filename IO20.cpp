...
template <typename T> bool read(std::vector<T> &s) { for (T &e : s) if (!read(e)) return false; return true; }
...
template <typename T> void print(const std::vector<T> &s) { print(s.front()); for (size_t i = 1, n = s.size(); i < n; ++i) putchar(' '), print(s[i]); }
void print_() { return; }
