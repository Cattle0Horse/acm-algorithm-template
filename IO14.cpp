...
template <typename T, is_int> bool read(T *s, const size_t &n) { for (size_t i = 0; i < n; ++i) if (!read(s[i])) return false; return true; }
template <typename T, is_int> bool read(std::vector<T> &s, const size_t &n) { for (size_t i = 0; i < n; ++i) if (!read(s[i])) return false; return true; }
...
template <typename T, is_int> void print(const T *s, const size_t &n) { print(s[0]); for (size_t i = 1; i < n; ++i) putchar(' '), print(s[i]); }
template <typename T, is_int> void print(const std::vector<T> &s, const size_t &n) { print(s[0]); for (size_t i = 1; i < n; ++i) putchar(' '), print(s[i]); }
template <typename T, is_int> void print(const std::vector<T> &s) { print(s, s.size()); }
void print_() { return; }
